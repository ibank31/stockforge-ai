...
The adapter speaks the public Gradio queue API used by Hugging Face Spaces and
can therefore target both a Hugging Face Space and a Kaggle-hosted Gradio
worker. Provider-specific GPU/model code stays inside the worker.
"""
...
class RemoteGradioProvider(GenerationProvider):
    ...
    def generate(self, request: GenerationRequest) -> GenerationResult:
        job = self.submit(request, provider_job_id=None)
    ...
    def submit(self, request: GenerationRequest, *, provider_job_id: str | None = None) -> ProviderJob:
        job_id = provider_job_id or self._new_job_id(request)
        ...
        try:
            ...
            event_id = str(event.get("event_id") or job_id)
            self._jobs[job_id] = ProviderJob(event_id, "submitted")
            return self._jobs[job_id]
        except Exception as exc:
            failed = ProviderJob(job_id, "failed", error_code=type(exc).__name__, error_message=str(exc))
    ...
    def status(self, provider_job_id: str) -> ProviderJob:
        ...
        if cached.state in {"succeeded", "failed", "cancelled"}:
            return cached
        return self._poll(provider_job_id, cached.provider_job_id)
    ...
    def _poll(self, durable_id: str, event_id: str) -> ProviderJob:
        ...
        if event == "complete":
            ...
            output = values[0]
            refs = self._materialize_outputs(output, durable_id)
            ...
            result = GenerationResult(
                ...
                model_id=None,
                seed=int(values[1]) if len(values) > 1 and values[1] is not None else None,
                parameters={"remote_provider": self.provider_id, "gpu_seconds": values[2] if len(values) > 2 else None},
            )
            job = ProviderJob(durable_id, "succeeded", result=result)
        else:
            message = data or f"Gradio worker returned event={event!r}"
            job = ProviderJob(durable_id, "failed", error_code="remote_generation_failed", error_message=message)
    ...
    def _materialize_outputs(self, output: Any, job_id: str) -> tuple[dict[str, Any], ...]:
        ...
        for index, item in enumerate(items):
            if not isinstance(item, dict):
                raise RemoteGradioError("Remote output is not a Gradio FileData object")
            url = item.get("url")
            if not url:
                raise RemoteGradioError("Remote output does not contain a downloadable URL")
            suffix = Path(str(item.get("orig_name") or "output.png")).suffix or ".png"
            filename = f"{job_id}-{index}{suffix}"
            target = self.output_dir / filename
            self._download(str(url), target)
