...
async function reconcileKaggle(env, job) {
  ...
  const finalResult = {
    ...
    model: String(remoteResult.model_id || "RealESRGAN_x4plus"),
    raw_r2_key: job.raw_r2_key,
    final: { provider: "kaggle-realesrgan", model: String(remoteResult.model_id || "RealESRGAN_x4plus"), scale: 4, width, height, megapixels: Number(megapixels.toFixed(4)), size_bytes: body.byteLength, sha256: hash, final_r2_key: finalKey, final_asset_url: `/api/assets/${job.id}?kind=final&token=${job.asset_token}` },
    ...
    technical_qa: { status: megapixels >= 16 ? "PASS_WITH_VISUAL_REVIEW" : "FAIL", format: "JPEG", color_space: "sRGB", dimensions: { width, height, megapixels: Number(megapixels.toFixed(4)) } },
    finalization: { mode: "ai_upscale", provider: "kaggle-realesrgan", algorithm: "Real-ESRGAN x4", source_dimensions: { width: Number(job.width || 0), height: Number(job.height || 0) }, output_dimensions: { width, height }, kaggle_version_number: result.kaggle_version_number || null },
  };
