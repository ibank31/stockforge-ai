...
async function recordEvent(env, jobId, type, stage, status, message, details = null) {
  await env.DB.prepare(`INSERT INTO job_events_sf(job_id,event_type,stage,status,message,details_json,created_at) VALUES(?,?,?,?,?,?,?)`).bind(jobId, type, stage || null, status || null, message || null, details ? JSON.stringify(details) : null, new Date().toISOString()).run();
}

async function reconcileKaggle(env, job) {
  if (!env.STOCKFORGE_KAGGLE_BRIDGE_URL || !job.result_json) return;
  let result = {};
  ...
  const base = String(env.STOCKFORGE_KAGGLE_BRIDGE_URL).replace(/\/$/, "");
  const response = await fetch(`${base}/upscale/status/${encodeURIComponent(providerJobId)}`, { headers: { "cache-control": "no-cache" } });
  if (!response.ok) return;
  const remote = await response.json();
  if (remote.state === "failed") {
    await env.DB.prepare(`UPDATE jobs_sf SET status=?,stage=?,error=?,failure_code=?,retryable=?,updated_at=? WHERE id=?`).bind("failed", "FAILED_UPSCALE", String(remote.error || "Kaggle upscale failed").slice(0, 2000), "KAGGLE_UPSCALE_FAILED", 1, new Date().toISOString(), job.id).run();
    await recordEvent(env, job.id, "upscale_failed", "FAILED_UPSCALE", "failed", String(remote.error || "Kaggle upscale failed"), { provider_job_id: providerJobId });
    return;
  }
  if (remote.state !== "completed" || !remote.result) return;

  const artifactResponse = await fetch(`${base}/upscale/artifact/${encodeURIComponent(providerJobId)}`);
  ...
  const final = remote.result.master || {};
  ...
  const finalResult = {
    ...
    model: String(remote.result.model_id || "RealESRGAN_x4plus"),
    ...
    technical_qa: {
      ...
      format: "JPEG",
      color_space: "sRGB",
      dimensions: { width, height, megapixels: Number(megapixels.toFixed(4)) },
    },
    finalization: {
      mode: "ai_upscale",
      provider: "kaggle-realesrgan",
      algorithm: "Real-ESRGAN x4",
      source_dimensions: { width: Number(job.width || 0), height: Number(job.height || 0) },
...
export async function onRequestGet(context) {
  try {
    ...
    if (["upscale_submitted", "upscaling"].includes(job.status)) {
      try { await reconcileKaggle(env, job); } catch (_) {}
      job = await env.DB.prepare(`SELECT * FROM jobs_sf WHERE id=?`).bind(String(params.jobId || "")).first();
    }
    let result = null;
