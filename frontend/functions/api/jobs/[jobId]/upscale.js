...
export async function onRequestPost(context) {
  try {
    ...
    try {
      workflowResponse = await env.STOCKFORGE_WORKFLOW.fetch(new Request("https://stockforge-pipeline/start", {
        method: "POST",
      ...
      await recordEvent(env, jobId, "upscale_dispatch_network_failed", "FAILED_DISPATCH", "failed", detail, { pipeline: "service-binding" });
      return json({ detail: "Unable to reach finalization pipeline", error: detail, retryable: true }, 502);
    }
    if (!workflowResponse.ok) {
      ...
      await recordEvent(env, jobId, "upscale_dispatch_failed", "FAILED_DISPATCH", "failed", "Unable to start finalization workflow.", { detail: detail.slice(0, 2000), pipeline: "service-binding" });
