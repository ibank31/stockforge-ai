...
export async function onRequestPost(context) {
  try {
    ...
    const t = now();
    const claim = await env.DB.prepare(`UPDATE jobs_sf SET status=?,stage=?,error=NULL,retryable=0,failed_mode=NULL,failure_code=NULL,upscale_attempts=upscale_attempts+1,updated_at=? WHERE id=? AND status='ready_upscale'`)
      .bind("upscale_submitted", "UPSCALING", t, jobId).run();
    ...
    await env.DB.prepare(`UPDATE workflows_sf SET status=?,stage=?,progress=?,message=?,updated_at=? WHERE reference_id=?`)
      .bind("running", "UPSCALING", 70, "4x finalization accepted; GPU work is running separately from generation.", t, job.reference_id).run();
    ...
    if (!workflowResponse.ok) {
      ...
      await env.DB.prepare(`UPDATE jobs_sf SET status=?,stage=?,error=?,failure_code=?,failed_mode=?,retryable=?,updated_at=? WHERE id=? AND status='upscale_submitted'`)
        .bind("failed", "FAILED_DISPATCH", detail.slice(0, 2000), "DISPATCH_ERROR", "upscale", 1, now(), jobId).run();
      await env.DB.prepare(`UPDATE workflows_sf SET status=?,stage=?,progress=?,message=?,updated_at=? WHERE reference_id=?`)
        .bind("ready", "READY_UPSCALE", 55, "Upscale start was rejected. The raw asset remains available for retry.", now(), job.reference_id).run();
    ...
    const result = {
      ...
      finalization: { mode: "upscale", workflow_instance_id: instanceId, status: "queued" },
    };
    await env.DB.prepare(`UPDATE jobs_sf SET status=?,stage=?,result_json=?,last_workflow_id=?,last_workflow_created_at=?,updated_at=? WHERE id=? AND status='upscale_submitted'`)
      .bind("upscale_submitted", "UPSCALING", JSON.stringify(result), instanceId, now(), jobId).run();
