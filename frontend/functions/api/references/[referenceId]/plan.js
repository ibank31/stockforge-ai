...
function parseJsonDeep(value, maxDepth = 5) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    try {
      ...
      return typeof parsed === "string" ? parseJsonDeep(parsed, maxDepth - 1) : parsed;
    } catch (_) {
...
function normalizeAnalysis(raw) {
  ...
  return {
    ...
    reference_facts: {
      subject: text(rf.subject || source.subject, "Unknown visible subject."),
      composition: text(rf.composition || source.composition, "Unknown composition."),
      viewpoint: text(rf.viewpoint || source.viewpoint, "Unknown viewpoint."),
      color_direction: text(rf.color_direction || rf.palette || source.color_direction || source.palette, "Unknown color direction."),
...
function differenceCount(refFacts, opp) {
  const pairs = [
    [refFacts.subject, opp.subject],
    [refFacts.composition, opp.composition],
    [refFacts.viewpoint, opp.viewpoint],
    [refFacts.color_direction, opp.color_direction],
    [refFacts.context, opp.context],
  ];
  return pairs.filter(([a, b]) => {
    if (!text(b)) return false;
    if (text(a).toLowerCase() === text(b).toLowerCase()) return false;
...
function buildPlan(referenceId, intent, opp, index, seed) {
  const rationale = opp.differences.length ? opp.differences : ["subject", "composition", "viewpoint"];
  const prompt = [
    `Create a commercially useful original stock image for: ${opp.use_case}.`,
    `New subject direction: ${opp.subject}.`,
    ...
    `No logos, brands, watermarks, creator names, earnings figures, readable pseudo-text, or copied layout.`,
    `Clean professional stock aesthetic, realistic lighting, technically coherent subject, useful negative space where appropriate.`,
  ].join(" ");
  return {
    schema_version: 3,
...
export async function onRequestPost(context) {
  try {
    ...
    if (!validation.passed) {
      return json({ detail: "Selected opportunity does not pass the minimum material-difference gate", validation, reference_facts: analysis.reference_facts, opportunity }, 422);
    }

    const plan = buildPlan(referenceId, text(body.market_intent, opportunity.use_case), opportunity, index, Number.isInteger(body.seed) ? body.seed : null);
    plan.reference_anchor = {
      subject: analysis.reference_facts.subject,
      composition: analysis.reference_facts.composition,
      viewpoint: analysis.reference_facts.viewpoint,
      color_direction: analysis.reference_facts.color_direction,
      context: analysis.reference_facts.context,
      commercial_signals: analysis.commercial_signals,
    };
    plan.validation = validation;

    await env.DB.prepare(`INSERT INTO plans_sf(reference_id,plan_json,created_at) VALUES(?,?,?) ON CONFLICT(reference_id) DO UPDATE SET plan_json=excluded.plan_json, created_at=excluded.created_at`)
      .bind(referenceId, JSON.stringify(plan), now()).run();
    await env.DB.prepare(`UPDATE workflows_sf SET status=?,stage=?,progress=?,message=?,updated_at=? WHERE reference_id=?`)
      .bind("ready", "PLANNED", 100, `Opportunity ${index + 1} is synchronized to the uploaded reference and passed the material-difference gate.`, now(), referenceId).run();

    return json({
      reference_id: referenceId,
      reference_anchor: plan.reference_anchor,
      candidates: analysis.asset_opportunities,
