...
**Updated:** 2026-08-25
**Branch:** `main`
**Latest verified commit:** `e082695` (WebP preview packaging fix); follow-up context-routing correction prepared locally; deployed Space commit: `935faa5`

**Current incident:** The endpoint deployment is healthy: Space runtime is `RUNNING`, domain is `READY`, and Gradio metadata exposes `/generate_remote`. The two authorized rotor-armature requests reached remote inference, but both Termux runs stopped locally while building the review package. The first exposed the missing WebP preview route; the second showed that the initial fix did not read the actual minimal `portfolio_snapshot` context, where delivery format is nested under `pre_gpu_gate.format_route`. No release package was returned and no further retry is allowed without a new explicit decision. The corrected context-routing patch is tested locally but not yet committed or deployed.
...
1. Commit and synchronize the corrected `pre_gpu_gate.format_route` handling; do not run another GPU request until this patch is deployed and verified.
...
The current main branch has passed **291 tests with 1 skipped** in the sandbox after the corrected WebP preview context-routing fix, with 49 non-blocking Pillow deprecation warnings. Syntax compilation and whitespace checks pass. The active ZeroGPU `app.py` regression passes, and the endpoint patch is deployed to Space commit `935faa5`; runtime `RUNNING`/domain `READY` and `/gradio_api/info` exposure of `/generate_remote` with seven parameters were verified. Both controlled trial requests reached remote inference according to HF logs, but the Termux release builder stopped before returning a package. The corrected local packaging fix now reads the actual `pre_gpu_gate.format_route`; it has passed the full suite but is not yet committed or deployed.
