...
**Updated:** 2026-08-25
**Branch:** `main`
**Latest verified commit:** `41034fb` (verified ZeroGPU deployment documentation); local packaging fix prepared after the controlled trial; deployed Space commit: `935faa5`

**Current incident:** The endpoint deployment is healthy: Space runtime is `RUNNING`, domain is `READY`, and Gradio metadata exposes `/generate_remote`. The one authorized rotor-armature trial then reached remote model loading and completed all 8 sampling steps according to HF container logs, but the Termux command exited locally while building the review package because the generated provider preview was `.webp` and `_technical_report_for` had no preview-aware route. No release package was returned and no retry was performed. A local patch now preserves WebP as review-only and defers the final JPEG gate to the master/finalization stage; it is tested locally but not yet deployed or used for another GPU request.
...
1. Review and explicitly approve deployment of the local WebP-preview packaging fix before any new GPU request; the previous single trial consumed the authorized request and did not return a release package.
2. Mature the JPEG route before any routine generation: evaluate the new `Technical Mechanical Component Illustrations` hypothesis against the existing nine lanes, select one globally researched buyer job, strengthen the scene prompt/layout contract, and keep one-candidate evidence rules.
3. Benchmark the prepared Kaggle Real-ESRGAN finalizer once the target runtime is available, comparing source/master at 100% for artifacts, texture, edges, color, dimensions, and sRGB.
4. Strengthen semantic/commercial QA for JPEG; the no-provider path must remain review/blocking rather than pretending anatomy, realism, or buyer usefulness passed.
5. Use the non-upload JPEG metadata preflight across supported platforms and the new nine-lane identity profiles; expand reviewed lane/category mappings only when the visual category is unambiguous, and keep title/keyword handling visual-first, platform-specific, and anti-spam.
6. Keep SVG value-upgrade and PNG alpha work frozen until the JPEG maturation checkpoint is complete; do not generate or upload them in this phase.
7. Keep model/provider registry, health, quota, and failover contracts explicit before adding more GPU providers.
...
The current main branch has passed **291 tests with 1 skipped** in the sandbox after the WebP preview packaging regression fix, with 49 non-blocking Pillow deprecation warnings. Syntax compilation and whitespace checks pass. The active ZeroGPU `app.py` regression passes, and the endpoint patch is deployed to Space commit `935faa5`; runtime `RUNNING`/domain `READY` and `/gradio_api/info` exposure of `/generate_remote` with seven parameters were verified. The controlled trial's HF logs show model loading and 8/8 sampling completion; the Termux failure occurred afterward in local release packaging. The new local packaging fix is not yet deployed and no follow-up generation has been run.
