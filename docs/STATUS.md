...
## SVG two-trial diagnosis — redesign required

The user reviewed two SVG trials and reported that both were not attractive enough and that many elements collided with surrounding lines. Diagnosis completed without building a third trial. Trial 1 (`file-flow-micro-set`) is technically clean but generic: eight broad UI primitives, large empty grid gaps, no workflow rhythm, and no differentiated buyer story. Trial 2 (`document-review-delivery-micro-set`) is more specific but visually cramped: thick strokes and arrowheads exceed the usable inner area of circular containers, and some document corners/actions merge with container rings. The current SVG validator checks XML/native elements and forbidden embeds but does not check stroke envelopes, minimum clearance, pairwise overlap, or thumbnail readability.

Root cause is design-system and QA mismatch, not raster/provider failure. Circular containers reduce usable space, while child geometry was sized as if on an open canvas; strokes are too heavy; icons are independent primitives rather than a composed workflow; semantics are redundant at thumbnail size; and transparent artwork on a dark viewer backdrop makes clearance harder to judge. The redesign direction is substantively different: treat the product as an editable workflow diagram kit rather than eight equal generic icons, use larger safe cells, lighter stroke hierarchy, fewer decorative rings, intentional sequence rhythm/connectors, explicit inner-boundary checks, pairwise overlap checks, and both white/checkerboard review renders.

No third SVG trial, upload package, or marketplace submission is authorized by this diagnosis. Diagnosis report: `docs/research/DOCUMENT_WORKFLOW_VECTOR_TRIAL_DIAGNOSIS_2026-08-25.md`.


## Monochrome vector quality audit — no new trial

The user observed that many Adobe Stock icons appear black and white and noted that both StockForge vector trials are far behind in quality. Official Adobe guidance says vectors are useful partly because customers can change icon colors, while its icon guidance prioritizes simplified communication, clarity, instant recognition, and themed consistency. Google Material Icons provides a concrete monochrome/tinting precedent. These facts explain why monochrome is common as a flexible base, but they do not prove that most Adobe icons are monochrome or that monochrome sells better.

The key failure in StockForge is not the use of color alone. Trial 1 is a generic primitive grid with semantic marks touching base shapes; Trial 2 adds decorative circular containers but does not calculate inner safe zones, stroke envelopes, or minimum clearance, causing arrowheads/document corners/strokes to collide with borders. XML/native-element validation passed because it does not equal visual QA.

No third trial was built. The redesign direction is monochrome-first/two-tone-neutral, remove decorative circles unless semantically useful, reduce stroke hierarchy, use consistent cap/join rules and larger safe cells, and add geometry QA for element bounds, stroke clearance, pairwise overlaps, and thumbnail readability before human review. Evidence and citations were appended to `docs/research/VECTOR_ROUTE_EVIDENCE_2026-08-25.md`; diagnosis remains `docs/research/DOCUMENT_WORKFLOW_VECTOR_TRIAL_DIAGNOSIS_2026-08-25.md`.


## Monochrome workflow diagram redesign — implementation and dry-run complete

After explicit user approval of the redesign, StockForge added a materially distinct lane: `native_vector_workflow_diagram_kits` / `document-lifecycle-diagram-kit`. This is not a silent mutation of either rejected trial. The product brief is a monochrome-first editable workflow diagram kit with six large independent modules: intake, organize, review, approve, archive, and deliver. It removes decorative circular badges, uses dark-neutral primary geometry with restrained neutral secondary/accent tones, and keeps five connector lanes outside the card interiors.

The local native SVG builder now supports preset `document_lifecycle_diagram_kit`. Each module has a named group, explicit card bounds, an inner safe-zone contract, and a measurable native glyph group. The validator now calculates conservative stroke-aware bounds for rects, circles, lines, polygons/polylines, and paths; checks glyph envelopes against safe zones; checks connector envelopes against card bounds; and requires exactly six cards and five connectors for this preset. The new test asserts native editability, six-card structure, named workflow groups, no raster/text, and a passing geometry-clearance report.

The selector now recommends the new diagram-kit lane for `icon_set`; the two earlier lanes remain historical/reference only. Focused tests passed for both the old historical trial and the new preset. Full suite passed `305 passed, 1 skipped, 49 warnings`; compileall and `git diff --check` passed. Readiness, plan-type, and trial-readiness dry-runs passed with `single_candidate_only=true`, `trial_allowed=true`, and `provider_call_allowed=false`; no SVG trial was built, no provider/GPU/Kaggle call was made, and no upload package or marketplace action occurred.


## Remote AI vector design research — no provider enabled

The user requested a way for an AI model outside the local deterministic builder to perform the design work because the local SVG output is not competitive enough. Research identified two hosted candidates with documented SVG-oriented generation: Recraft V4.1 Vector via REST API and QuiverAI Text-to-SVG via API/MCP. Recraft documents `/v1/images/generations/vector`, vector model IDs, editable/recolorable output, and published charges of USD 0.08/image for standard Vector and USD 0.30/image for Pro Vector. QuiverAI documents `POST /v1/svgs/generations`, Arrow 1.1/Arrow 1.1 Max, instructions, references, and editable SVG output. The existing QuiverAI connector is present but disabled and authorization-required; no Recraft connector exists. No credential was requested, connector was enabled, or provider call was made.

Adobe Firefly remains a quality benchmark: its web/Illustrator Text to Vector feature exports editable SVG, but the official Firefly Services API reference currently documents image, video, composite, and upscale operations rather than a verified Text-to-Vector/SVG endpoint. StarVector and OmniSVG are open/research models with text/image-to-SVG capabilities, but require a separate GPU/runtime and commercial/data-rights review; they are not lightweight drop-in production routes.

Recommended architecture is a separate remote-vector adapter, not a silent change to the local route or raster Gradio worker: compile one brief, call one remote vector generation, preserve provenance, quarantine and sanitize SVG, enforce native-element/group/external-reference/dimension/stroke/geometry/thumbnail checks, render a review image, then ask for human verdict. A provider output that is only PNG, flattened, or unsafe must fail closed. Full comparison and references are in `docs/research/REMOTE_VECTOR_AI_OPTIONS_2026-08-25.md`. This research does not prove demand, sales, ranking, conversion, or marketplace acceptance.
