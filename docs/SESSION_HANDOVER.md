...
Branch remote dan lokal hanya `main`. Folder-upload SVG lane changes are committed at `28e1c85`, and the single trial evidence is committed and pushed at `ace69e7` on `main`, on top of the rejected modular-ribbon evidence. Current SVG trial evidence is stored under `trial_outputs/folder_upload_svg/` and `trial_outputs/svg_modular_ribbon/`. Test suite terakhir: **273 passed, 1 skipped**. Remote Gradio adapter memakai endpoint `generate_remote` dengan durable `stockforge_job_id`; ZeroGPU deployment memakai `remote_api.py` sebagai entrypoint programmatic.
...
The asset-type selector, readiness report, trial-readiness guard, and native SVG pattern/object presets are complete. The rejected local modular-ribbon SVG trial remains evidence: structural status PASS, commercial status rejected at **2/10** because the buyer could not identify the object or its use. The research-backed `folder-upload` hypothesis was then executed as exactly one local candidate: execution `f397114e-179e-4992-a1e2-cae0d819d934`, artifact `282ff154-112b-4203-acf3-92a1098987ba`, structural status PASS, visual/commercial status `REVIEW_REQUIRED`. Evidence is in `trial_outputs/folder_upload_svg/` and `docs/research/FOLDER_UPLOAD_PRETRIAL_SPEC_2026-08-24.md`. Do not mark it upload-ready or claim portal acceptance; wait for the user's review before writing an evaluation decision. PNG remains blocked until an alpha-capable path and portal validation are complete.
