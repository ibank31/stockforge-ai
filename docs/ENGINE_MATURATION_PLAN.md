...
Stage 1 is complete. The selector, readiness report, and `portfolio plan-type` entry point are implemented and tested without generation. Supported types now include `scene`, `native_object`, `icon_set`, `technical_icon`, `seamless_pattern`, and `transparent_cutout`. Native SVG has a dedicated `native_vector_elements` lane led by the research-backed `folder-upload` single-object baseline, plus a separate `native_vector_utility_sets` lane for the higher-value eight-action file-flow micro-set hypothesis. Legacy modular-ribbon and technical-badge briefs remain for historical/regression comparison. Both SVG routes are deterministic and native-only; the micro-set uses an explicit cluster contract and has not been generated yet. PNG has a conservative true-alpha normalizer that refuses opaque sources, but the production route remains explicitly blocked. Unsupported asset types fail closed.

## Current next action

Complete the global market-intelligence contract and buyer-value gates, then prepare exactly one local file-flow micro-set trial against the existing folder-upload baseline. The rejected `modular-ribbon` trial remains evidence that structural SVG validity is not buyer-fit, while the folder-upload trial remains the single-icon control. `portfolio plan-type --asset-type <type>` is the no-generation entry point, and `portfolio trial-readiness` remains the explicit guard before any provider call while enforcing one candidate per hypothesis. The micro-set route is local and does not need a provider; it must not be generated until the product specification, semantic gates, preview plan, and market evidence are documented. PNG remains blocked until a real alpha-capable producer, anti-fringe/trim policy, and portal evidence exist.
