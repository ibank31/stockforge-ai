...
Stage 1 is complete. The selector, readiness report, and `portfolio plan-type` entry point are implemented and tested without generation. Supported types are `scene`, `native_object`, `technical_icon`, `seamless_pattern`, and `transparent_cutout`. Native SVG now has a dedicated `native_vector_elements` lane led by the research-backed `folder-upload` single-object brief, with legacy modular-ribbon and technical-badge briefs retained for historical/regression comparison. The folder-upload preset is deterministic, native-only, square-framed, high-contrast, and tied to the file-management/cloud-workflow buyer job. PNG has a conservative true-alpha normalizer that refuses opaque sources, but the production route remains explicitly blocked. Unsupported asset types fail closed.

## Current next action

Complete the semantic and visual review gates around the research-backed `folder-upload` brief before authorizing its one-candidate local SVG trial. The rejected `modular-ribbon` trial remains evidence that structural SVG validity is not buyer-fit. `portfolio plan-type --asset-type <type>` is the no-generation entry point for future user selection, and `portfolio trial-readiness` remains the explicit guard before any provider call while enforcing one candidate per hypothesis. The folder-upload route is local and does not need a provider; no folder-upload trial has been run yet. PNG remains blocked until a real alpha-capable producer, anti-fringe/trim policy, and portal evidence exist. Any generation trial becomes justified only after the exact hypothesis, semantic gates, and review purpose are documented first.
